...
export default function LiveComments({ channel }: Props) {
  ...
  return (
    <section className="border border-blue-200 dark:border-blue-900 rounded-lg p-4 bg-blue-50 dark:bg-[#0a1a2b]">{/* visible container */}
      <h2 className="font-medium mb-3 text-blue-900 dark:text-blue-200">Live Comments</h2>
      <div className="flex gap-2 mb-4">
        <input
          className="flex-1 rounded border border-blue-200 dark:border-blue-900 px-3 py-2 text-sm bg-white dark:bg-[#0f2033] text-blue-950 dark:text-blue-100 placeholder:text-blue-400"
          value={input}
      ...
      {!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ? (
        ...
        <ul className="space-y-3">
          {comments.map((c) => (
            <li key={c.id} className="border border-blue-200 dark:border-blue-900 rounded p-3 bg-white dark:bg-[#0f2033]">
